...
def sample(net, num_samples, dim, num_steps = 100, device = 'cuda:0'):
...
#     if i == 0: print('Using Heun 2nd order method')
#     x_cur = x_next
...
#     # DEBUG INFO
#     if i % 10 == 0:  # print setiap 10 step
#         print(f"Step {i}: t_cur={t_cur:.3f}, ||x_hat||={x_hat.norm():.3f}, ||denoised||={denoised.norm():.3f}, ||d_cur||={d_cur.norm():.3f}")
#         print(f"         x_hat min/max = {x_hat.min():.3f} / {x_hat.max():.3f}")
#         print(f"         denoised min/max = {denoised.min():.3f} / {denoised.max():.3f}")
...
def sample_step(net, num_steps, i, t_cur, t_next, x_next):
    if i == 0: print('Using Euler method')
    x_cur = x_next
