...
def train_val_test_split(data_df, cat_columns, num_train = 0, num_test = 0):
    ...
    while True:
        ...
        for i in cat_columns:
            # print(set(train_df[i]), set(data_df[i]))
...
def process_data(name):
    ...
    if info['file_type'] == 'csv':
        if info['name'] in ['canada','fiji','uk','rwanda','indonesia','adulta','churn']:
            data_df = pd.read_csv(data_path, dtype=str)
            num_columns = [data_df.columns[i] for i in info['num_col_idx']]

            if info['name'] in ['canada','fiji','uk','rwanda','indonesia']:
                data_df['AGE'] = data_df['AGE'].replace(['91-92', '93-94', '95+'], ['92', '94', '95'])
            
                data_df[num_columns] = data_df[num_columns].astype(float)
                iimp = IterativeImputer(
                    estimator = xgboost.XGBRegressor(),
                    random_state = 42,
                    verbose = 2,
                )
                    ## imputation to remove NA
                data_df[num_columns] = iimp.fit_transform(data_df[num_columns]) 
            data_df = data_df.fillna('nan')
        else: data_df = pd.read_csv(data_path, header = info['header'])
    ...
    if info['test_path']:
        ...
        if info['name'] not in ['canada','fiji','uk','rwanda','indonesia','adulta','churn','tcga','tcgaa','diabetes']:
