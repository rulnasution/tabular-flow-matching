...
class VAE(nn.Module):
    def __init__(self, d_numerical, categories, num_layers, hid_dim, n_head = 1, factor = 4, bias = True):
        ...
        if d_numerical > 256:
            d_num_start = 256
            self.num_linear_proj = nn.Sequential(
                nn.LayerNorm(d_numerical),
                nn.Linear(d_numerical, d_num_start),
            )
        else:
            d_num_start = d_numerical

        self.Tokenizer = Tokenizer(d_num_start, categories, d_token, bias = bias)
    ...
    def forward(self, x_num, x_cat):
        if self.d_numerical > 256:
            x1 = self.num_linear_proj(x_num)
            x = self.Tokenizer(x1, x_cat)
        else:
...
class Reconstructor(nn.Module):
    def __init__(self, d_numerical, categories, d_token):
        ...
        if d_numerical > 256:
            self.d_num_start = 256
            self.num_linear_proj = nn.Sequential(
                nn.LayerNorm(self.d_num_start),
                nn.Linear(self.d_num_start, d_numerical),
            )
        else:
            self.d_num_start = d_numerical
        
        self.weight = nn.Parameter(Tensor(self.d_num_start, d_token))  
    ...
    def forward(self, h):
        h_num  = h[:, :self.d_num_start]
        h_cat  = h[:, self.d_num_start:]

        recon_x_num = torch.mul(h_num, self.weight.unsqueeze(0)).sum(-1)
        if self.d_numerical > 256:
            recon_x_num = self.num_linear_proj(recon_x_num)
...
class Encoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True):
        ...
        if d_numerical > 256:
            ...
            d_num_start = d_numerical
        self.Tokenizer = Tokenizer(d_num_start, categories, d_token, bias)
        self.VAE_Encoder = Transformer(num_layers, d_token, n_head, d_token, factor)

    def load_weights(self, Pretrained_VAE):
        if self.d_numerical > 256:
            self.num_linear_proj.load_state_dict(Pretrained_VAE.VAE.num_linear_proj.state_dict())
